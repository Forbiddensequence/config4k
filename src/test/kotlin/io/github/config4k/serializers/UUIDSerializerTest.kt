...
class UUIDSerializerTest {
    ...
    private object CustomUUIDSerializer : KSerializer<UUID> by stringSerializer(
        decode = { UUID.fromString(it.removeSuffix("-custom")) },
        encode = { "$it-custom" },
        nameOverride = "custom.name.UUID",
    )

    @Nested
    inner class CustomUUIDTest {
        @Test
        fun checkDefaultDescriptor() {
            assertThat(UUIDSerializer.descriptor.serialName).isEqualTo("java.util.UUID")
            assertThat(UUIDSerializer.descriptor.kind).isEqualTo(PrimitiveKind.STRING)
        }

        @Test
        fun checkSerialNameOverriding() {
            assertThat(CustomUUIDSerializer.descriptor.serialName).isEqualTo("custom.name.UUID")
        }
        ...
        fun checkDecoding() {
            @Serializable
            data class Conf(
                @Serializable(with = CustomUUIDSerializer::class) val u: UUID,
            )

            val test: Conf = Config4k.decodeFromConfig("u = b14240e7-acc0-4a19-bc5d-31a7901e36b0-custom".toConfig())
        ...
        fun checkEncoding() {
            ...
            assertThat(test.render()).isEqualTo("u=b14240e7-acc0-4a19-bc5d-31a7901e36b0-custom")
        }
    }
