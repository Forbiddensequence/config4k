 ...
 * )
 * val config = ConfigFactory.parseString("regex = a = [{key = 5, value = foo},{key = 6, value = bar}]")
...
public class MapAsListSerializer<T, V>(
    ...
    override val descriptor: SerialDescriptor =
        PrimitiveSerialDescriptor("hocon.kotlin.collections.Map", PrimitiveKind.STRING)
        ...
        if (decoder is HoconDecoder) {
            decoder.decodeConfigValue { conf, path ->
                when (val type = conf.getValue(path).valueType()) {
                    ...
                    else -> throw SerializationException("hocon.kotlin.collections.Map can't be specified by $type value type")
                }
            }
        } else {
